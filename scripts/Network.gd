...
remote func _send_player_info(id, info):
	players[id] = info
	var new_player = load('res://scenes/controls/player/Player.tscn').instance()
	new_player.name = str(id)
	new_player.set_network_master(id)
	get_tree().get_root().get_node("GameScreen").add_child(new_player)
	#$'/root/scenes/GameScreen/'.add_child(new_player)
